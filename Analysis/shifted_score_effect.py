...
def peak_ambiguity_row(index, data_dict_filtered, cachedStructures_filtered, siriusDirectory, helpers, matches_array):
    ...
    result = {"matched": len(site_locator.shifted), "score": post_sirius}
...
if __name__ == '__main__':
    ...
    try:
        ...
        res = multiprocessing_wrapper(peak_ambiguity_row, library = "BERKELEY-LAB", accepted_adduct = "M+H", count = 2500, processes = 8)
        
        # create pandas dataframe
        df = pd.DataFrame(res)
        df.to_csv("matched.csv")
    ...
    # df = df.groupby(pd.cut(df["matched"], np.arange(0, 100, 5))).mean()
    ...
    # plot data
    plt.scatter(matchedPeaks, scores, s=190)
    ...
    plt.xlabel("# shifted peaks", fontsize=140)
